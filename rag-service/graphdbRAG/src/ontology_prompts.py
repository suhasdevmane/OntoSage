...
5. Prefixed names (e.g., `__NS_PREFIX__:Formaldehyde_Level_Sensor_5.05`) must remain identical
If a sensor appears as `__NS_PREFIX__:Formaldehyde_Level_Sensor_5.05` in a triple, you MUST output `__NS_PREFIX__:Formaldehyde_Level_Sensor_5.05` (not a spaced version like `FORMALDEHYDE LEVEL SENSOR 5.05`).
...
4. Preserve all RDF prefixes AND the local part of the identifier exactly (including underscores and periods) as they appear (brick:, bot:, saref:, __NS_PREFIX__:, etc.) - these are critical for SPARQL generation.
...
Example Pattern (TTL):
- Entity subject: __NS_PREFIX__:CO_Level_Sensor_5.06
  linked to _:genid179 with type `ref:TimeseriesReference`,
  having `ref:hasTimeseriesId "791284f8-..."` and `ref:storedAt __NS_PREFIX__:database1`.
...
- External Reference tuple:
  ...
  Where:
    - entity_name: exact TTL identifier of the entity (e.g., __NS_PREFIX__:CO_Level_Sensor_5.06)
...
text:
RDF Triples:
__NS_PREFIX__:TempSensor_R101 rdf:type brick:Temperature_Sensor .
__NS_PREFIX__:TempSensor_R101 brick:isPointOf __NS_PREFIX__:AHU_01 .
__NS_PREFIX__:AHU_01 rdf:type brick:AHU .
__NS_PREFIX__:AHU_01 brick:feeds __NS_PREFIX__:Zone_West .
__NS_PREFIX__:Zone_West rdf:type brick:HVAC_Zone .
output:
("entity"{TUPLE_DELIMITER}__NS_PREFIX__:TempSensor_R101{TUPLE_DELIMITER}TemperatureSensor{TUPLE_DELIMITER}A temperature sensor located in Room 101, measuring air temperature as a point of AHU_01{TUPLE_DELIMITER}bldg){RECORD_DELIMITER}
("entity"{TUPLE_DELIMITER}__NS_PREFIX__:AHU_01{TUPLE_DELIMITER}AHU{TUPLE_DELIMITER}Air Handling Unit 01, an HVAC equipment that serves the west zone and has multiple sensor points{TUPLE_DELIMITER}bldg){RECORD_DELIMITER}
("entity"{TUPLE_DELIMITER}__NS_PREFIX__:Zone_West{TUPLE_DELIMITER}Zone{TUPLE_DELIMITER}West HVAC zone in the building, served by AHU_01{TUPLE_DELIMITER}bldg){RECORD_DELIMITER}
...
("relationship"{TUPLE_DELIMITER}__NS_PREFIX__:TempSensor_R101{TUPLE_DELIMITER}brick:Temperature_Sensor{TUPLE_DELIMITER}rdf:type{TUPLE_DELIMITER}TempSensor_R101 is an instance of the Brick Temperature_Sensor class{TUPLE_DELIMITER}10){RECORD_DELIMITER}
("relationship"{TUPLE_DELIMITER}__NS_PREFIX__:TempSensor_R101{TUPLE_DELIMITER}__NS_PREFIX__:AHU_01{TUPLE_DELIMITER}brick:isPointOf{TUPLE_DELIMITER}The temperature sensor is a measurement point of AHU_01{TUPLE_DELIMITER}9){RECORD_DELIMITER}
("relationship"{TUPLE_DELIMITER}__NS_PREFIX__:AHU_01{TUPLE_DELIMITER}brick:AHU{TUPLE_DELIMITER}rdf:type{TUPLE_DELIMITER}AHU_01 is an instance of the Brick AHU class{TUPLE_DELIMITER}10){RECORD_DELIMITER}
("relationship"{TUPLE_DELIMITER}__NS_PREFIX__:AHU_01{TUPLE_DELIMITER}__NS_PREFIX__:Zone_West{TUPLE_DELIMITER}brick:feeds{TUPLE_DELIMITER}AHU_01 supplies conditioned air to the West zone{TUPLE_DELIMITER}9){RECORD_DELIMITER}
("relationship"{TUPLE_DELIMITER}__NS_PREFIX__:Zone_West{TUPLE_DELIMITER}brick:HVAC_Zone{TUPLE_DELIMITER}rdf:type{TUPLE_DELIMITER}Zone_West is an instance of the Brick HVAC_Zone class{TUPLE_DELIMITER}10){COMPLETION_DELIMITER}
...
"""
    ...
    return ChatPromptTemplate.from_template(_localise(prompt_template))


# ── The ACTIVE building's namespace, for the worked SPARQL examples ─────────
#
# The examples declared `PREFIX __NS_PREFIX__: <__BUILDING_NS__>` and
# `PREFIX __NS_PREFIX__: <__BUILDING_NS__>`. A model shown three examples using a
# namespace reaches for it -- and that one matches nothing in any graph, so a query built
# on it returns ZERO ROWS AND NO ERROR. That is indistinguishable from a building with no
# such sensors, which is the same failure the self-correction fallback had (fixed the same
# day) one level up.
#
# Substituted with a NON-BRACE marker on purpose: `{building_ns}` inside a
# ChatPromptTemplate becomes a required input variable and would break every caller.
#
# Empty rather than defaulted. An example with a blank namespace is visibly incomplete; a
# plausible wrong one is not.
_BUILDING_NS = (os.getenv("BUILDING_NAMESPACE", "") or "").strip()
_BUILDING_PREFIX = (os.getenv("BUILDING_PREFIX", "bldg") or "bldg").strip()


def _localise(template: str) -> str:
    """Put this building's prefix and namespace into the worked examples."""
    return template.replace("__NS_PREFIX__", _BUILDING_PREFIX).replace(
        "__BUILDING_NS__", _BUILDING_NS
    )
...
-Context-
You have access to:
1. Entities: Building components with their types and prefixes (e.g., __NS_PREFIX__:TempSensor_R101, brick:Temperature_Sensor)
...
4. Prefixes: Available namespaces (brick:, bot:, saref:, __NS_PREFIX__:, rdf:, rdfs:)
...
2. Map query terms to ontology concepts:
   ...
   - "in the west zone" → __NS_PREFIX__:Zone_West with bot:containsZone
...
Context entities: __NS_PREFIX__:TempSensor_R101, __NS_PREFIX__:Zone_West, brick:Temperature_Sensor
Context relationships: brick:isPointOf, brick:feeds, bot:containsZone
Available prefixes: brick:, bot:, __NS_PREFIX__:, rdf:, rdfs:, ashrae:, ref:

Output:
```sparql
PREFIX brick: <https://brickschema.org/schema/Brick#>
PREFIX bot: <https://w3id.org/bot#>
PREFIX __NS_PREFIX__: <__BUILDING_NS__>
...
Context entities: __NS_PREFIX__:Floor_01, __NS_PREFIX__:AHU_01, __NS_PREFIX__:VAV_101, bot:Storey
Context relationships: bot:hasStorey, bot:containsElement, rdf:type
Available prefixes: brick:, bot:, __NS_PREFIX__:, rdf:
...
WHERE {{
  __NS_PREFIX__:Floor_01 rdf:type bot:Storey .
  __NS_PREFIX__:Floor_01 bot:containsElement ?equipment .
...
Context entities: __NS_PREFIX__:CO_Level_Sensor_5.06, brick:CO_Level_Sensor, brick:HVAC_Zone
Available prefixes: brick:, bot:, __NS_PREFIX__:, ashrae:, ref:, rdf:, rdfs:
...
WHERE {{
    ...
    FILTER(STR(?sensor) = STR(__NS_PREFIX__:CO_Level_Sensor_5.06))
    OPTIONAL {{
        ?sensor ashrae:hasExternalReference|ref:hasExternalReference ?ref .
        ?ref ref:hasTimeseriesId ?timeseriesId .
        ?ref ref:storedAt ?storedAt .
