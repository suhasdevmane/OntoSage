...
# The live admin password used to be written out below. It was redacted on 2026-09-10,
# when un-ignoring tasks/ (CAVEAT-501) would have committed it permanently. Source it
# from .env instead:  export $(grep -E '^ADMIN_(USERNAME|PASSWORD)=' .env | xargs)
# Current admin (dev): admin@ontosage / $ADMIN_PASSWORD
TOK=$(curl -s -X POST http://127.0.0.1:3001/auth/login -H 'Content-Type: application/json' \
  -d '{"username":"admin@ontosage","password":"$ADMIN_PASSWORD"}' \
