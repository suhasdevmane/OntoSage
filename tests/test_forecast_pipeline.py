...
def _make_series(n: int = 72, freq: str = "1h", noise: float = 0.5) -> pd.Series:
    ...
    idx = pd.date_range(start=datetime(2026, 5, 1, tzinfo=timezone.utc), periods=n, freq=freq)
...
def test_linear_forecaster_ci_ordering():
    ...
    for i in range(10):
        assert (
            result["lower_95"][i] <= result["lower_80"][i]
        ), f"95% lower exceeds 80% lower at step {i}"
        assert (
            result["upper_95"][i] >= result["upper_80"][i]
        ), f"95% upper below 80% upper at step {i}"
...
def test_arima_forecaster_output_shape():
    ...
    from orchestrator.services.forecasting.models.arima_forecaster import (
        ARIMAForecaster,
    )
...
async def test_forecast_agent_no_data():
    ...
    assert (
        "not available" in result["formatted_response"].lower()
        or "no sensor" in result["formatted_response"].lower()
    )
...
async def test_forecast_agent_response_contains_table():
    ...
    state = ConversationState(
        ...
        messages=[
            Message(role="user", content="forecast temperature next 6 hours", timestamp=dt.now())
        ],
        building_id="bldg1",
    )
    records = _make_records(n=72)
    ...
    result = await agent.predict(
        state,
        "forecast temperature next 6 hours",
        {"data": records},
        {"uuid-temp-001": {"label": "Temperature"}},
    )
