...
def test_sensor_data_intent(chat_client, fresh_session_id):
    ...
    assert resp.contains_any("°C", "temperature", "sensor", "reading", "specific room"), (
...
def test_analytics_intent(chat_client, fresh_session_id):
    ...
    assert resp.success
    assert resp.contains_any("average", "spread", "°C", "zone", "compliance", "sensor"), (
...
def test_discovery_intent(chat_client, fresh_session_id):
    ...
    assert resp.success
    assert resp.contains_any("sensor", "temperature", "co2", "humidity", "types"), (
...
def test_report_intent(chat_client, fresh_session_id):
    ...
    assert resp.success
    assert resp.contains_any("report", "summary", "building", "abacws"), (
...
def test_anomaly_intent(chat_client, fresh_session_id):
    ...
    assert resp.success
    assert resp.contains_any("anomal", "spike", "deviation", "no significant", "sensor"), (
...
def test_comparison_intent(chat_client, fresh_session_id):
    ...
    assert resp.success
    assert resp.contains_any("floor 1", "floor 3", "co2", "ppm", "compare", "compliance"), (
...
def test_export_intent(chat_client, fresh_session_id):
    ...
    assert resp.success
    assert resp.contains_any("export", "csv", "rows", "bytes", "complete"), (
...
def test_forecast_intent(chat_client, fresh_session_id):
    ...
    assert resp.success
    assert resp.contains_any("forecast", "predict", "temperature", "trend", "current"), (
...
def test_recommend_intent(chat_client, fresh_session_id):
    ...
    assert resp.contains_any("ashrae", "setpoint", "recommend", "temperature", "zone"), (
...
def test_alert_intent(chat_client, fresh_session_id):
    ...
    assert resp.success
    assert resp.contains_any("co2", "ppm", "threshold", "alert", "sensor"), (
...
def test_floor_plan_intent(chat_client, fresh_session_id):
    ...
    resp = chat_client.chat(
        "Show me the floor plan for floor 3", session_id=fresh_session_id
    )
    assert resp.success
    ...
    assert resp.contains_any("floor 3", "floor plan", "third floor", "room", "floor"), (
...
def test_spatial_query_intent(chat_client, fresh_session_id):
    ...
    resp = chat_client.chat(
        "How many rooms are on the second floor?", session_id=fresh_session_id
    )
    assert resp.success
    assert resp.contains_any("room", "floor 2", "second floor", "count", "zone"), (
...
def test_general_intent(chat_client, fresh_session_id):
    ...
    assert resp.success
    assert resp.contains_any("help", "building", "ontosage", "ask", "questions"), (
...
def test_clarification_intent(chat_client, fresh_session_id):
    ...
    resp = chat_client.chat(
        "What was the reading again?", session_id=fresh_session_id
    )
    assert resp.success
    ...
    assert resp.contains_any(
        "specific", "could you", "what", "more", "please",
        "reading", "sensor", "zone", "building", "clarify",
    ), (
...
def test_control_intent(chat_client, fresh_session_id):
    ...
    resp = chat_client.chat(
        "Turn off the lights in room 3.01", session_id=fresh_session_id
    )
    assert resp.success
...
def test_capability_intent_baseline(chat_client, fresh_session_id):
    ...
    assert resp.success
    assert resp.contains_any("evacuation", "fire", "assembly point", "exit"), (
