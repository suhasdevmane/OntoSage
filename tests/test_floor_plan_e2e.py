...
SYNTHETIC_TEXT_BLOCKS = [
    {
        "text": "3.01",
        "centroid": {"x": 0.20, "y": 0.30},
        "bbox": {"x": 0.15, "y": 0.25, "w": 0.10, "h": 0.10},
    },
    {
        "text": "3.02",
        "centroid": {"x": 0.40, "y": 0.30},
        "bbox": {"x": 0.35, "y": 0.25, "w": 0.10, "h": 0.10},
    },
    {
        "text": "3.12",
        "centroid": {"x": 0.60, "y": 0.50},
        "bbox": {"x": 0.55, "y": 0.45, "w": 0.10, "h": 0.10},
    },
    {
        "text": "Server Room",
        "centroid": {"x": 0.80, "y": 0.70},
        "bbox": {"x": 0.75, "y": 0.65, "w": 0.15, "h": 0.10},
    },
    {
        "text": "Meeting Room A",
        "centroid": {"x": 0.20, "y": 0.70},
        "bbox": {"x": 0.15, "y": 0.65, "w": 0.15, "h": 0.10},
    },
    {
        "text": "Kitchen",
        "centroid": {"x": 0.40, "y": 0.70},
        "bbox": {"x": 0.35, "y": 0.65, "w": 0.10, "h": 0.10},
    },
]
...
class TestPipelineE2E:
    ...
    async def test_ingest_produces_manifest(self, pipeline, tmp_building):
        with (
            patch(
                "orchestrator.services.floor_plan_pipeline._render_pdf", side_effect=_patched_render
            ),
            patch(
                "orchestrator.services.floor_plan_pipeline._extract_text_blocks",
                side_effect=_patched_extract,
            ),
            patch(
                "orchestrator.services.floor_plan_pipeline._run_in_executor",
                new=_run_in_executor_passthrough,
            ),
    ...
    async def test_ingest_extracts_facilities(self, pipeline, tmp_building):
        ...
        assert (
            "server_room" in types or "meeting_room" in types
...
class TestFloorPlanAgentE2E:
    ...
    async def test_where_is_server_room_finds_floor(self, pipeline, manifest_on_disk):
        ...
        assert (
            result.floor is not None or "server" in result.markdown.lower()
