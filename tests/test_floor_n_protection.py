...
def test_temperature_floor_3_routes_to_sensor_data(chat_client, fresh_session_id):
    ...
    assert resp.contains_any("°c", "temperature", "sensor", "floor 3", "reading"), (
...
def test_analytics_floor_2_routes_to_analytics(chat_client, fresh_session_id):
    ...
    resp = chat_client.chat(
        "Show me analytics for floor 2 sensors", session_id=fresh_session_id
    )
    assert resp.success
    assert not resp.contains("I have floor plans for"), (
...
def test_sensor_count_floor_1_routes_to_discovery(chat_client, fresh_session_id):
    ...
    resp = chat_client.chat(
        "How many CO2 sensors are on floor 1?", session_id=fresh_session_id
    )
    assert resp.success
    assert not resp.contains("I have floor plans for"), (
    ...
    assert resp.contains_any("sensor", "co2", "floor 1", "count", "first floor"), (
