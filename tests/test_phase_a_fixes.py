...
class TestUUIDExtraction:
    @pytest.mark.parametrize("val,should_match", [
...
class TestFallbackFilename:
    def test_fallback_uses_uuid_not_current_data(self):
        ...
        assert "current_data.json" not in except_block, (
            "Fallback in except block still uses 'current_data.json' — concurrent requests will collide."
        )
        assert "uuid4" in except_block or "fallback_" in except_block, (
...
class TestNewIntentsRouting:
    ...
    @pytest.mark.parametrize("intent,expected_dest", [
        ("greeting", "response"),
        ("unknown", "response"),
