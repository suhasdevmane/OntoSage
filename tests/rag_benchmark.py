...
BENCHMARK_QUESTIONS = [
    # ── Analytics (dynamic data) ──────────────────────────────────────────────
    {
        ...
        "expected_entities": ["zone 1"],
        "required_analytics": ["latest"],
    },
    {
        "id": "A02",
        "intent": "analytics",
        "query": "What is the average CO2 level on floor 2 today?",
        "expected_entities": ["floor 2"],
    ...
    },
    {
        ...
        "expected_entities": ["Air_Temperature_Sensor_1_01"],
        "required_analytics": [],
    },
    {
        "id": "M03",
    ...
    },
    {
        ...
        "expected_entities": ["Air_Temperature_Sensor_1_02"],
        "required_analytics": [],
    },
    # ── Discovery ────────────────────────────────────────────────────────────
    {
    ...
    # ── Compare ──────────────────────────────────────────────────────────────
    {
        ...
        "expected_entities": ["zone 1", "zone 2"],
        "required_analytics": ["avg"],
    },
    {
        "id": "C02",
...
def compute_entity_recall(expected: List[str], found: List[str]) -> float:
    ...
    if not expected:
        return 1.0
    hits = sum(
        1 for e in expected if any(e.lower() in f.lower() or f.lower() in e.lower() for f in found)
    )
...
async def run_mock_benchmark(questions: List[Dict]) -> List[BenchmarkResult]:
    ...
    for q in questions:
        ...
        # Response quality
        r.response_quality = round(min(5.0, max(1.0, random.gauss(QUALITY_MEAN, 0.4))), 1)
        results.append(r)
    return results
...
async def run_live_benchmark(
    questions: List[Dict], base_url: str = "http://localhost:8000"
...
def build_report(results: List[BenchmarkResult], total_time: float) -> Dict:
    ...
    return {
        ...
        "overall": {
            ...
            "sparql_success_rate": (
                round(sparql_ok / len(sparql_tests) * 100, 1) if sparql_tests else None
            ),
        ...
        "failures": [
            ...
            for r in results
            if r.error or not r.intent_correct
        ],
    }
...
def format_markdown_report(report: Dict) -> str:
    ...
    if report["failures"]:
        ...
        for f in report["failures"]:
            lines.append(
                f"| {f['id']} | {f['query']} | `{f['expected']}` | `{f.get('got', 'N/A')}` | {f.get('error') or ''} |"
            )
...
async def main(args):
    ...
    for r in results:
        ...
        print(
            f"  {icon} [{r.question_id}] {r.query[:55]:<55} "
    ...
    print(
        f"  p50 / p90 / p99 : {ov['latency_p50_ms']} / {ov['latency_p90_ms']} / {ov['latency_p99_ms']} ms"
    )
    print(f"{'─'*60}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--intent", default=None, help="Filter to a single intent (e.g. analytics, report)"
    )
    parser.add_argument("--url", default="http://localhost:8000", help="Live orchestrator base URL")
    parser.add_argument(
        "--output",
        default="outputs/benchmark/rag_benchmark_report.md",
