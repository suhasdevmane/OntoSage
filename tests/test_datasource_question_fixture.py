...
def _load_datasource_ids() -> List[str]:
    data = yaml.safe_load(DATASOURCES_PATH.read_text(encoding="utf-8"))
...
class TestFixtureCoverage:
    ...
    def test_all_lockable_sources_have_at_least_one_entry(self) -> None:
        ...
        assert not uncovered, (
            f"Lockable sources not covered by any fixture question: {uncovered}"
        )
    ...
    def test_at_least_one_no_lock_question_per_unlockable_source(self) -> None:
        ...
        covered = {
            q["source_id"]
            for q in qs
            if q["when_off"].get("behavior") == "pass_through"
        }
    ...
    def test_multiple_personas_represented(self) -> None:
        ...
        assert len(personas) >= 3, (
            f"Fixture should represent >= 3 personas, only has: {personas}"
        )
