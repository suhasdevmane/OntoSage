...
------------------
  ...
  T7_hardest_per_persona.csv          the single hardest question per persona
  ...
  F5_persona_difficulty.png           mean latent level by persona
...
def main() -> None:
    ...
    stats = {
        ...
        "n_personas": int(df["personas"].nunique()),
    ...
    # ------------------------------------------------------------------ #
    cols = [
        "qid", "personas", "stage", "question", "level", "level_name", "latent_level",
    ...
    persona_grp.to_csv(OUTDIR / "T6_difficulty_by_persona.csv")

    hardest_per_persona = (
        df.sort_values("difficulty", ascending=False).groupby("personas").head(1)
    )[["personas", "question", "level", "latent_level", "difficulty", "answerability", "new_items_str"]]
    hardest_per_persona.sort_values("difficulty", ascending=False).to_csv(
        OUTDIR / "T7_hardest_per_persona.csv", index=False
    )
    ...
    # F5 — persona difficulty (personas with >= 80 questions)
    def short_persona(r: str) -> str:
        parts = [p.strip() for p in r.split(";")]
        first = parts[0].split("/")[0].strip()
        if len(parts) > 1:
            return f"{first} +{len(parts) - 1} personas"
        return parts[0][:38]

    big = persona_grp[persona_grp["n"] >= 80].sort_values("mean_latent")
    ...
    ax.set_yticks(y)
    ax.set_yticklabels(
        [f"{short_persona(r)}  (n={int(n)})" for r, n in zip(big.index, big["n"])], fontsize=8
    )
    ax.set_xlabel("mean level (1-6)")
    ax.set_title("Question difficulty by asker persona — lay users implicitly demand\n"
                 "more system architecture than professionals")
    ax.legend(loc="lower right")
    fig.tight_layout()
    fig.savefig(OUTDIR / "F5_persona_difficulty.png", bbox_inches="tight")
    plt.close(fig)
