...
Outputs:
  outputs/tables/D1_persona_domain_counts.csv
  outputs/tables/D1_persona_domain_pct.csv
  outputs/tables/D1_chi_squared_results.md
  outputs/figures/D1_persona_domain_heatmap.{png,pdf}
  outputs/tables/D2_concordance_table.csv
  outputs/tables/D2_persona_topic_rank.csv
  outputs/figures/D2_rank_by_persona.{png,pdf}
  outputs/tables/D3_user_personas.md

Methods:
  D1 — Chi-squared independence (persona x domain), Cramer's V
  D2 — Per-persona mean rank for each of 20 topics; Kendall's W (overall and per-persona);
       Spearman correlation matrix between persona rank-vectors
  D3 — Persona synthesis derived from persona aggregates (top domains, complexity mix,
...
def persona_domain_tables(corpus: pd.DataFrame, user_summary: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    df = corpus.merge(user_summary[["PID", "PrimaryPersona"]], on="PID", how="left")
    counts = pd.crosstab(df["PrimaryPersona"], df["domain_l1"])
    pct = counts.div(counts.sum(axis=1), axis=0).mul(100).round(2)
    return counts, pct


def chi_squared_persona_domain(counts: pd.DataFrame) -> str:
    chi2, p, dof, _ = stats.chi2_contingency(counts.values)
    n = int(counts.values.sum())
    r, c = counts.shape
    cramers_v = float(np.sqrt(chi2 / (n * (min(r, c) - 1))))
    lines = [
        "# Phase D1 — Chi-squared independence: persona x domain",
        "",
        f"- Chi^2({dof}) = {chi2:.3f}",
        f"- p = {p:.4g}",
        f"- N = {n} questions",
        f"- Personas (rows) = {r}, Domains (cols) = {c}",
        f"- Cramer's V = {cramers_v:.4f}",
        "",
        "Interpretation: Cramer's V scale (Cohen 1988) — 0.10 small, 0.30 medium, 0.50 large.",
        "Significant chi-squared (p < 0.05) confirms that question-domain distribution depends on user persona.",
        "",
    ]
    return "\n".join(lines) + "\n"


def plot_persona_domain_heatmap(pct: pd.DataFrame) -> None:
    pct = pct.copy()
    ...
    sns.heatmap(
        ...
        fmt=".1f",
        cmap="Blues",
        cbar_kws={"label": "% of persona's questions"},
    )
    plt.title("Question domain mix by user persona (top-12 domains, OTHER excluded)")
    plt.xlabel("Domain")
    plt.ylabel("Primary persona")
    plt.tight_layout()
    plt.savefig(OUT / "figures" / "D1_persona_domain_heatmap.png", dpi=300)
    plt.savefig(OUT / "figures" / "D1_persona_domain_heatmap.pdf")
    plt.close()


# ---------------------------------------------------------------------------
# D2 — Persona x Topic Ranking
# ---------------------------------------------------------------------------


def long_topic_rankings(topic_df: pd.DataFrame, user_pid: pd.DataFrame, user_summary: pd.DataFrame) -> pd.DataFrame:
    """Reshape wide Rank_K_ID columns into (PID, Persona, TopicID, TopicLabel, Rank)."""
    rows = []
...
def persona_topic_table(long_df: pd.DataFrame) -> pd.DataFrame:
    return (
        long_df.groupby(["PrimaryPersona", "TopicLabel"])["Rank"]
        .mean()
...
def persona_concordance(long_df: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for persona, sub in long_df.groupby("PrimaryPersona"):
...
def plot_rank_by_persona(persona_topic: pd.DataFrame) -> None:
    keep_personas = persona_topic.dropna(thresh=10).index.tolist()
    if not keep_personas:
        return
    sub = persona_topic.loc[keep_personas].copy()
    ...
    plt.title("Mean topic rank by primary persona (lower = higher priority)")
    plt.xlabel("Topic")
    plt.ylabel("Primary persona")
    plt.xticks(rotation=45, ha="right", fontsize=8)
    plt.tight_layout()
    plt.savefig(OUT / "figures" / "D2_rank_by_persona.png", dpi=300)
    plt.savefig(OUT / "figures" / "D2_rank_by_persona.pdf")
    plt.close()
...
def synthesize_personas(
    ...
    persona_topic: pd.DataFrame,
    concordance: pd.DataFrame,
) -> str:
    df = corpus.merge(user_summary[["PID", "PrimaryPersona"]], on="PID", how="left")
    on_topic = df[df["domain_l1"] != "OTHER"]
    eligible = (
        df.groupby("PrimaryPersona")["PID"].nunique().sort_values(ascending=False).head(6)
    )
    blocks = ["# Phase D3 — User Personas (synthesised from corpus + topic rankings)\n"]
    for persona, n_users in eligible.items():
        sub = df[df["PrimaryPersona"] == persona]
        sub_on = on_topic[on_topic["PrimaryPersona"] == persona]
        top_domains = (
        ...
        if persona in persona_topic.index:
            ranks = persona_topic.loc[persona].dropna().sort_values()
        ...
        kw = concordance[concordance["persona"] == persona]
        ...
        block = [
            ...
            + (
                ...
                if len(top_topics)
                else "n/a (no ranking data for this persona)"
            ),
            f"- **Within-persona agreement (Kendall's W):** "
...
def append_summary(pct: pd.DataFrame, concordance: pd.DataFrame) -> None:
    ...
    top_persona = pct.sum(axis=1).sort_values(ascending=False).index[0] if len(pct) else "?"
    persona_count = len(pct)
    avg_w = concordance["kendalls_w"].dropna().mean()
    line = (
        f"- Phase D: {persona_count} persona categories analysed; "
        f"largest persona = {top_persona}; "
        f"mean within-persona Kendall's W = {avg_w:.3f}.\n"
    )
    if path.exists():
...
def main() -> None:
    ...
    counts, pct = persona_domain_tables(corpus, user_summary)
    counts.to_csv(OUT / "tables" / "D1_persona_domain_counts.csv")
    pct.to_csv(OUT / "tables" / "D1_persona_domain_pct.csv")
    md = chi_squared_persona_domain(counts)
    (OUT / "tables" / "D1_chi_squared_results.md").write_text(md, encoding="utf-8")
    plot_persona_domain_heatmap(pct)

    long_df = long_topic_rankings(topic_df, user_pid, user_summary)
    persona_topic = persona_topic_table(long_df)
    persona_topic.to_csv(OUT / "tables" / "D2_persona_topic_rank.csv")
    concordance = persona_concordance(long_df)
    concordance.to_csv(OUT / "tables" / "D2_concordance_table.csv", index=False)
    plot_rank_by_persona(persona_topic)

    personas_md = synthesize_personas(corpus, user_summary, persona_topic, concordance)
    (OUT / "tables" / "D3_user_personas.md").write_text(personas_md, encoding="utf-8")

    append_summary(pct, concordance)

    print("Phase D done.")
    print("Persona x domain (top-3 domains per persona):")
    for persona in pct.index:
        top = pct.loc[persona].sort_values(ascending=False).head(3)
        print(f"  {persona}: " + ", ".join(f"{d} {v:.1f}%" for d, v in top.items()))
    print()
    print("Concordance:")
