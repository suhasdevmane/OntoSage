...
_HORIZON_RULES: list[tuple[str, timedelta, timedelta, str, str]] = [
    ...
    (r"next\s+month|over\s+the\s+next\s+month|coming\s+month",
     timedelta(days=30),  timedelta(hours=12), "next 30 days",  "12h"),
    (r"next\s+week|coming\s+week|over\s+the\s+next\s+week",
     timedelta(days=7),   timedelta(hours=6),  "next 7 days",   "6h"),
    (r"next\s+3\s+days|coming\s+3\s+days|over\s+the\s+next\s+3\s+days",
     timedelta(days=3),   timedelta(hours=3),  "next 3 days",   "3h"),
    (r"next\s+2\s+days|tomorrow\s+and\s+after",
     timedelta(days=2),   timedelta(hours=2),  "next 48 hours", "2h"),
    (r"tomorrow|next\s+24\s+hours|over\s+the\s+next\s+24\s+hours",
     timedelta(hours=24), timedelta(hours=1),  "next 24 hours", "1h"),
    (r"next\s+12\s+hours|coming\s+12\s+hours",
     timedelta(hours=12), timedelta(minutes=30), "next 12 hours", "30min"),
    (r"next\s+8\s+hours|coming\s+8\s+hours",
     timedelta(hours=8),  timedelta(minutes=30), "next 8 hours",  "30min"),
    (r"next\s+6\s+hours|coming\s+6\s+hours|this\s+afternoon",
     timedelta(hours=6),  timedelta(minutes=30), "next 6 hours",  "30min"),
    (r"next\s+4\s+hours|coming\s+4\s+hours",
     timedelta(hours=4),  timedelta(minutes=30), "next 4 hours",  "30min"),
    (r"next\s+2\s+hours|coming\s+2\s+hours",
     timedelta(hours=2),  timedelta(minutes=15), "next 2 hours",  "15min"),
    (r"next\s+hour|coming\s+hour|next\s+60\s+min",
     timedelta(hours=1),  timedelta(minutes=10), "next hour",     "10min"),
    (r"next\s+30\s+min(?:utes?)?",
     timedelta(minutes=30), timedelta(minutes=5),  "next 30 minutes", "5min"),
]
