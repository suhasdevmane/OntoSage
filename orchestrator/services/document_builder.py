...
class DocumentBuilder:
    ...
    def render(
        ...
        try:
            ...
            tz = (
                ZoneInfo(settings.BUILDING_TIMEZONE)
                if hasattr(settings, "BUILDING_TIMEZONE")
                else None
            )
            generated_at = (
            ...
            ctx = {
                ...
                or report_data.get("formatted_text")
                or "",
                "readings": report_data.get("readings_summary")
                or report_data.get("readings")
                or {},
    ...
    def _render_docx(self, report_data: Dict, ctx: Dict, filename: str) -> Dict[str, Any]:
        ...
        try:
            ...
            run = meta.add_run(
                f"{ctx['building_name']} | {ctx['generated_at']} | {ctx['report_type'].title()} Report"
            )
            run.font.size = Pt(10)
            ...
            run = footer.add_run(
                f"Generated by OntoSage | {ctx['building_name']} | {ctx['generated_at']}"
            )
            run.font.size = Pt(8)
            ...
            return {
                "success": False,
                "error": "python-docx not installed; install it for DOCX output",
                "fallback": True,
            }
