...
class RedisManager:
    ...
    async def load_state(self, conversation_id: str) -> Optional[ConversationState]:
        ...
        try:
            ...
            if state_json:
                ...
                logger.info(
                    f"🆕 REDIS LOAD: No state found for {conversation_id} (new conversation)"
                )
                return None
    ...
    async def save_user_preferences(self, user_id: str, preferences: Dict[str, Any]):
        ...
        try:
            ...
            await self.client.setex(key, 86400 * 30, json.dumps(preferences))  # 30 days
    ...
    async def get_user_preferences(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if prefs_json:
                ...
                # Default preferences
                return {"persona": "guest", "language": "en", "building_id": settings.BUILDING_ID}
        except Exception as e:
            logger.error(f"Failed to get preferences: {e}")
            return {}

    async def cache_sparql_result(self, query: str, result: Any, ttl: int = 3600):
        """
    ...
    async def add_conversation_to_user(self, user_id: str, conversation_id: str, title: str):
        ...
        try:
            ...
            # Store metadata
            meta = {"title": title, "updated_at": datetime.utcnow().isoformat()}
        ...
        try:
            ...
            for cid in conv_ids:
                ...
                if meta:
                    ...
                    conversations.append(
                        {"conversation_id": cid, "title": "Untitled", "updated_at": None}
                    )
