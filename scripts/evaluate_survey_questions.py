...
def send_chat_request(
    ...
    headers = {
        ...
        "Authorization": "Bearer evaluation_script"
    }
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate OntoSage system using a CSV of survey questions.")
    parser.add_argument(
        "--input", 
        default=os.path.join("paper", "Survey analysis and results", "inputs", "questions_only.csv"),
        help="Path to the input CSV file containing questions (one per line)."
    )
    parser.add_argument(
        "--output", 
        default=os.path.join("paper", "Survey analysis and results", "outputs", "survey_evaluation_results.csv"),
    ...
    parser.add_argument(
        ...
        type=int, 
        default=0,
        help="Only process this many questions (0 for all)."
    )
    parser.add_argument(
        "--delay", 
        type=float, 
    ...
    if output_exists:
        try:
            ...
            print(f"?? Found existing output file. Skipping {len(processed_questions)} already processed questions.")
        except Exception as e:
            print(f"?? Warning: Could not read existing output file ({e}). Starting fresh or appending.")
