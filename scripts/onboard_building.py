...
def validate_ttl(path: str) -> dict:
    ...
    try:
        ...
        classes = set(str(o) for s, p, o in g.triples((None, None, None))
...
def run_interactive():
    ...
    namespace = prompt("Ontology namespace URI (must end with '#')", default=f"http://example.com/{bldg_id}#")
    ...
    abox_file = prompt("Path to ABox TTL file (building instances)", default=f"data/{bldg_id}_abox.ttl")
    ...
    config_data = {
        ...
        "ontology": {
            "schema": schema,
            "schema_uri": "https://brickschema.org/schema/Brick#" if schema == "brick"
        ...
        "storage": {
            ...
            "database": database,
            "table": db_table,
            "columns": {"uuid": col_uuid, "value": col_val,
                        "timestamp": col_time, "sensor_name": "sensor_name"},
        },
    }
    config_yaml = generate_config(config_data)
    out_path = prompt("Output config file path",
                      default=f"config/{bldg_id}_building_config.yaml")
...
def run_non_interactive(args):
    ...
    config_data = {
        ...
        "storage": {
            ...
            "database": args.id,
            "table": "sensor_data",
            "columns": {"uuid": "uuid", "value": "value",
                        "timestamp": "time", "sensor_name": "sensor_name"},
        },
    }
