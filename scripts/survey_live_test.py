        ...
        if _attempt == 0:
            try:
                requests.post(
                    ...
                    headers={"Content-Type": "application/json"},
                    json={
                        "username": _SURVEY_USER,
                        "password": _SURVEY_PASS,
...
chat(
    "What is the current temperature reading for Air_Temperature_Sensor_5.28?",
...
chat(
    "Compare average temperature today for Zone 5.28 vs Zone 5.12.",
...
chat(
    "Show temperature trends for Air_Temperature_Sensor_5.28 over the past 24 hours.",
...
chat(
    "Generate a thermal comfort report for Zone 5.28 covering the last 7 days with anomaly flags.",
...
chat(
    "What is the latest CO2 reading from CO2_Sensor_5.08?",
...
chat(
    "Compare average CO2 today for Zone 5.08 vs Zone 5.10.",
...
chat(
    "Show CO2 trends for CO2_Sensor_5.08 over the last 7 days. Is ventilation adequate?",
...
chat(
    "Scan all CO2 sensors for readings above 1000 ppm in the last 12 hours.",
...
_safe_print("\n[T3] Humidity")
chat(
    "What is the average humidity today in Zone 5.06?",
...
chat(
    "Show humidity trends for Zone 5.06 over the past week.",
...
chat(
    "Are there any temperature anomalies in Zone 5.28 today? When did they occur?",
...
chat(
    "Run anomaly detection across all sensor types for the last 24 hours. Summarise by zone.",
...
_safe_print("\n[T5] Discovery & Ontology")
chat(
    "List all temperature sensors in the Abacws building.",
...
chat(
    "List all HVAC equipment in the building with their zones.",
...
_safe_print("\n[T6] Analytics")
chat(
    "Calculate the average, min, and max temperature for Air_Temperature_Sensor_5.28 today.",
...
chat(
    "Give me the last 5 temperature readings for Air_Temperature_Sensor_5.28 with timestamps.",
...
chat(
    "Show the hourly temperature profile for Zone 5.28 over the last 48 hours.",
...
chat(
    "Correlate temperature and occupancy data for Zone 5.28 over the last 24 hours.",
...
chat(
    "Which rooms are adjacent to room 3.01?",
...
chat(
    "Can I bring a guest or external visitor to the building?",
...
r_e2 = chat(
    "Show me analytics for CO2 sensors on floor 2.",
...
r_e4 = chat(
    "Compare energy usage on floor 1 vs floor 3 last month.",
...
for lbl, resp in [("EDGE-1", r_e1), ("EDGE-2", r_e2), ("EDGE-3", r_e3), ("EDGE-4", r_e4)]:
    if resp and any(x in resp.lower() for x in ["manifest", ".png", "room list", "floor_plan"]):
        ...
        for res in RESULTS:
            if (
                res["label"] == f"{lbl} temp floor3 must NOT route floor_plan"
                or lbl in res["label"]
            ):
                res["tier"] = "WARN"
                res["pass"] = False
...
_safe_print("\n[T10] Report Generation")
chat(
    "Generate a daily building health report covering temperature, CO2, energy and humidity for today.",
    "RPT-1 daily health report",
    expected_keywords=["report", "temperature", "building"],
)

chat(
    "Export the last 24 hours of temperature sensor data for Zone 5.28 as CSV.",
    "RPT-2 export CSV 24h",
    expected_keywords=["export", "csv", "temperature", "zone", "data"],
)

chat(
    "Detect and report all anomalies across all sensors in the last 48 hours.",
    "RPT-3 anomaly report 48h",
    expected_keywords=["anomal", "sensor", "report"],
)

chat(
    "Forecast temperature trends for Zone 5.28 for the next 7 days.",
...
_safe_print("\n[T11] Persona-aligned queries")
chat(
    "As a facility manager, show me a weekly maintenance and fault summary for all HVAC units.",
    "PERSONA-1 FM maintenance",
    expected_keywords=["maintenance", "hvac", "summary", "zone"],
)

chat(
    "I need to run a statistical analysis of CO2 sensor variance by floor for my research paper.",
...
chat(
    "I am a building safety officer. What are the current fire safety risks and sensor anomalies?",
    "PERSONA-3 safety officer",
    expected_keywords=["fire", "safety", "anomal", "sensor"],
)

chat(
    "As an energy manager, forecast energy consumption trends for next month based on current patterns.",
    "PERSONA-4 energy manager forecast",
    expected_keywords=["energy", "forecast", "trend", "consumption"],
)

chat(
    "I'm a student researcher studying occupancy patterns. Show me weekly occupancy data by floor.",
...
chat(
    "Give me a building health scorecard rating temperature comfort, air quality, "
...
chat(
    "Set the temperature setpoint to 22 degrees for Zone 5.28.",
...
chat(
    "Override the HVAC schedule and switch to eco mode tonight.",
...
RESULTS.append(
    {
        ...
        "query": "",
        "status": "PASS (422)" if _r1_ok else f"FAIL (HTTP {_r1.status_code})",
        "tier": "PASS" if _r1_ok else "FAIL",
        "response_len": 0,
        "response_preview": f"HTTP {_r1.status_code}",
        "elapsed": 0,
        "pass": _r1_ok,
        "warn": False,
    }
...
_safe_print(
    f"  {'[PASS]' if _r1_ok else '[FAIL]'} ROBUST-1 empty query: HTTP {_r1.status_code} (422=correct)"
)
...
if _r5 is not None:
    ...
    _last = RESULTS[-1]
    _is_crash = any(
        x in _last.get("response_preview", "").lower()
...
chat(
    "Things seem off today. Fix everything.",
...
chat(
    "Is it warm in room 5.28 right now?",
...
chat(
    "Does the air in the building feel okay today?",
...
chat(
    "Can you show me the layout of floor 3?",
...
chat(
    "Is there a lift I can use to get to the upper floors?",
...
chat(
    "How do I report that a light is broken on floor 2?",
...
chat(
    "Room 5.28 felt really warm this morning. Was the temperature higher than usual there?",
...
chat(
    "The meeting rooms on floor 5 always seem stuffy. Is the CO2 high there today "
...
chat(
    "I have a meeting in room 3.01 this afternoon. Is it comfortable temperature-wise "
...
chat(
    "Is the temperature and air quality okay in the main office areas today? "
...
chat(
    "How do I print something here and is the temperature okay where the printers are?",
...
chat(
    "Yesterday afternoon the whole of floor 5 felt terrible — way too hot and the air "
...
chat(
    "I need to book the most comfortable room for a 3-hour workshop tomorrow. "
...
chat(
    "It has been a rough week in this building. Can you give me a summary of "
...
chat(
    "Room 5.28 has felt uncomfortable all day. First tell me the current temperature "
...
chat(
    "We are planning a building open day next week. Can you tell me: "
...
chat(
    "I am evaluating this system. What ontology schema does it use, what sensor "
...
chat(
    "Give me the complete building topology: list every floor, the zones on each floor, "
...
chat(
    "What is the temporal coverage of the time-series database? "
...
chat(
    "Which zones have full multi-sensor coverage — meaning they have at least one "
...
chat(
    "Calculate z-score normalised anomaly scores for every temperature sensor "
...
chat(
    "Perform a multivariate correlation analysis: for zones that have both temperature "
...
chat(
    "Compute the 5th, 25th, 50th, 75th, and 95th percentile temperature readings "
...
chat(
    "Use CO2 rate-of-change (delta per 15-minute interval) as an occupancy proxy. "
...
chat(
    "Run a rolling 4-hour standard deviation analysis on all CO2 sensors for the "
...
chat(
    "I need to assess data quality across the whole building before I build a "
...
chat(
    "ASHRAE 55 thermal comfort compliance audit: "
...
chat(
    "Cross-floor comparative analytics: "
...
chat(
    "I want to set up a predictive maintenance early-warning system. "
...
chat(
    "Generate a comprehensive building intelligence report that I can present "
