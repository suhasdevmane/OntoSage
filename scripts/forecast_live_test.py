...
def chat(message, session_id=None, timeout=60):
    ...
    try:
        ...
        return {"ok": False, "intent": "ERROR", "response": str(e),
                "elapsed": round(time.time() - t0, 2), "http": 0}
...
check(r, "T1 temp-5.01 predict tomorrow", {
    "intent=trend":     lambda resp, intent: "trend" in intent,
    "has_prediction":   lambda resp, _: any(x in resp for x in ["°C", "°c", "Predicted", "Forecast", "predict"]),
    "has_ci_table":     lambda resp, _: "| Time |" in resp or "80%" in resp or "95%" in resp,
    "has_model_info":   lambda resp, _: any(x in resp for x in ["ARIMA", "Linear", "Holt", "Model Selection", "Winner"]),
    "has_metrics":      lambda resp, _: any(x in resp for x in ["RMSE", "MAE", "R²"]),
    "no_error":         lambda resp, _: "not available" not in resp.lower() and "error" not in resp.lower()[:50],
})
...
check(r, "T2 temp-5.02 next 24 hours", {
    "intent=trend":     lambda resp, intent: "trend" in intent,
    "has_prediction":   lambda resp, _: any(x in resp for x in ["°C", "Predicted", "forecast", "Forecast"]),
    "has_ci_or_model":  lambda resp, _: any(x in resp for x in ["80%", "95%", "ARIMA", "Linear", "Holt", "Winner"]),
})

# Test 3: Temperature 5.03 — next week
r = chat("Forecast temperature readings for Air Temperature Sensor 5.03 for next week")
check(r, "T3 temp-5.03 next week", {
    "intent=trend":     lambda resp, intent: "trend" in intent,
    "has_prediction":   lambda resp, _: any(x in resp for x in ["°C", "Predicted", "forecast", "Forecast", "Model"]),
...
check(r, "T4 co2-5.01 predict tomorrow", {
    "intent=trend":     lambda resp, intent: "trend" in intent,
...
check(r, "T5 co2-5.02 next 6 hours", {
    "intent=trend":     lambda resp, intent: "trend" in intent,
...
check(r, "T6 temp-5.01 next hour (short horizon)", {
    "intent=trend":     lambda resp, intent: "trend" in intent,
    "has_prediction":   lambda resp, _: len(resp) > 80,
    "short_steps":      lambda resp, _: any(x in resp for x in ["10min", "10 min", "minute", "next hour", "1 hour"]),
})

# Test 7: Natural language variation
r = chat("What will the temperature be in zone 5.01 tomorrow morning?")
check(r, "T7 NL variation 'what will be tomorrow'", {
    "intent=trend":     lambda resp, intent: "trend" in intent,
    "has_prediction":   lambda resp, _: len(resp) > 100,
    "has_ci_or_model":  lambda resp, _: any(x in resp for x in ["80%", "95%", "ARIMA", "Linear", "Model"]),
})

# Test 8: Trend-as-question format
r = chat("Is the CO2 level in zone 5.01 expected to rise over the next week?")
check(r, "T8 'expected to rise' CO2 next week", {
    "intent=trend":     lambda resp, intent: "trend" in intent,
    "has_prediction":   lambda resp, _: len(resp) > 100,
})
...
r1 = chat("Show me the temperature readings for Air Temperature Sensor 5.01 over the last 24 hours",
          session_id=MULTI_SESSION)
check(r1, "T9a multi-turn: recent data (step 1)", {
    "intent=sensor/analytics": lambda resp, intent: any(x in intent for x in ["sensor", "analyt", "trend", "metadata"]),
    "has_temperature":          lambda resp, _: any(x in resp.lower() for x in ["temperature", "°c", "23", "22", "24"]),
})

# Step 2: In same session, ask for forecast
r2 = chat("Based on those readings, predict what the temperature will be tomorrow",
          session_id=MULTI_SESSION)
check(r2, "T9b multi-turn: forecast after data (step 2)", {
    "intent=trend":     lambda resp, intent: "trend" in intent,
    "has_prediction":   lambda resp, _: len(resp) > 100,
})
...
check(r, "T10 explicit ARIMA request", {
    "intent=trend":     lambda resp, intent: "trend" in intent,
    "has_arima_or_pred":lambda resp, _: any(x in resp for x in ["ARIMA", "Predicted", "°C", "forecast", "arima"]),
    "has_content":      lambda resp, _: len(resp) > 100,
})

# Test 11: Linear Regression explicitly
r = chat("Using linear regression, forecast the temperature for Air Temperature Sensor 5.01 over the next 12 hours")
check(r, "T11 explicit Linear Regression request", {
    "intent=trend":     lambda resp, intent: "trend" in intent,
    "has_linear_or_pred":lambda resp, _: any(x in resp for x in ["Linear", "linear", "Predicted", "°C", "forecast"]),
    "has_content":      lambda resp, _: len(resp) > 100,
})

# Test 12: Exponential Smoothing explicitly
r = chat("Use exponential smoothing to predict temperature for zone 5.01 tomorrow")
check(r, "T12 explicit Holt-Winters ES request", {
    "intent=trend":     lambda resp, intent: "trend" in intent,
    "has_es_or_pred":   lambda resp, _: any(x in resp for x in ["Exponential", "Holt", "exponential", "Predicted", "°C"]),
    "has_content":      lambda resp, _: len(resp) > 100,
})
...
check(r, "T13 specific date: 3 June 2026", {
    "has_content":      lambda resp, _: len(resp) > 80,
    "not_crash":        lambda resp, _: "500" not in resp and "error" not in resp.lower()[:30],
})

# Test 14: Combined recent + future in one query
r = chat("Show me the temperature trend for sensor 5.01 over the last 7 days and predict the next 3 days")
check(r, "T14 combined past trend + future prediction", {
    "has_content":      lambda resp, _: len(resp) > 150,
    "has_data_or_pred": lambda resp, _: any(x in resp for x in ["°C", "temperature", "Temperature", "trend", "predict"]),
})

# Test 15: Non-existent zone (error path)
r = chat("Predict temperature for sensor 9.99 tomorrow")
check(r, "T15 non-existent sensor (graceful error)", {
...
check(r, "T16 all CO2 sensors floor 5 next 24h", {
    "has_content":      lambda resp, _: len(resp) > 80,
...
r = chat("Predict temperature for Air Temperature Sensor 5.01 for the next 24 hours with confidence intervals")
resp = r.get("response", "")
...
resp = r.get("response", "")
has_accuracy = any(x in resp for x in ["RMSE", "MAE", "MAPE", "accuracy", "reliable", "Reliability"])
check(r, "T18 accuracy metrics explicitly requested", {
    "intent=trend":     lambda resp, intent: "trend" in intent,
