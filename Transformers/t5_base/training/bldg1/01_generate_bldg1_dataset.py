"""Generate an extensive NL->SPARQL training dataset for the Building 1 Brick graph.

Output JSON (array of objects):
  {
	"question": str,
	"entities": [str, ...],  # prefixed names (bldg:, brick:)
	"sparql": str,           # SPARQL body (no PREFIX block inserted; intentionally blank)
	"category": str,         # template / intent category
	"notes": str             # short rationale or disambiguation
  }

Adapted from bldg2 generator with Building 1 namespace:
  http://abacwsbuilding.cardiff.ac.uk/abacws#  (prefix bldg:)
...
Coverage:
  ...
  - Class-centric queries (if classes resolvable in graph): list + count
  - Multi-class comparisons
  - Missing external reference quality checks
  - Equipment points filtered by class
  - Random lexical variation for richer training signal

CLI Example:
  python generate_bldg1_dataset.py --ttl bldg1.ttl --out bldg1_dataset_extended.json --limit 6000 --pretty \
	 --sensor-list sensors_list.txt --max-class-queries 40 --max-class-comparisons 32

Notes:
  - No UUID wording in natural language questions.
  - PREFIX lines omitted (add before execution if needed).
  - If the TTL lacks inline Brick class definitions, class-based templates may reduce; base templates still work.
"""
...
def sanitize_ttl(raw: str) -> str:
	...
	pattern = re.compile(r"\bbldg:([A-Za-z0-9_.-]*?/[\w\-./]*)")
	replacements = {}
...
def extract_graph_entities(g: Graph) -> Tuple[List[Equip], List[Point]]:
	...
	for s, _, o in g.triples((None, BRICK.hasPoint, None)):
...
class TemplateGenerator:
	...
	# ---------- Extended Templates ----------
	def t_class_points_list(self, class_key: str):
	...
	def all_templates(self):
		...
		extra = [self.t_missing_external_refs]
		return base, extra
...
def generate_dataset(g: Graph, limit: int, sensor_names: List[str], max_class_queries: int, max_class_comparisons: int) -> List[Dict]:
	...
	if not points:
		print("[WARN] No points discovered via brick:hasPoint in graph; dataset will be empty.")
		return []
	...
	# Class list + count queries
	for ck in selected_for_class_queries:
		q, ents, sp, cat, notes = tg.t_class_points_list(ck)
	...
	# Cycle base templates until limit
...
def main():
	...
	parser.add_argument('--ttl', default='bldg1.ttl')
	parser.add_argument('--out', default='bldg1_dataset_extended.json')
	parser.add_argument('--limit', type=int, default=6000)
	parser.add_argument('--pretty', action='store_true')
	parser.add_argument('--sensor-list', default='sensors_list.txt')
	parser.add_argument('--seed', type=int, default=42)
	parser.add_argument('--max-class-queries', type=int, default=40)
	parser.add_argument('--max-class-comparisons', type=int, default=32)
